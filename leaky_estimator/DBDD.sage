...
load("../leaky_estimator/load_strategies.sage")
load("../leaky_estimator/DBDD_generic.sage")
...
class DBDD(DBDD_generic):
    ...
    def integrate_approx_hint_fulldim(self, A, z, center,
        ...
        if not aposteriori:
            ...
            zero_m = vec(A.ncols() * [0])
            A = block4(A, -z.T, zero_m, vec([1]))

            zero = vec(covariance.nrows() * [0])
            F = (A * self.S * A.T + block4(covariance, zero.T, zero, vec([1]))).inverse()
            F[-1, -1] = 0
            c = vec(concatenate(center, 1))
            self.mu +=  (self.S * A.T * (F * (c.T - A * (self.mu).T))).T
            self.S -= self.S * A.T * F * A * self.S
        else:
            raise NotImplementedError()
